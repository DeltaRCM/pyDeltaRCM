...
class BaseRouter:
    ...
    def _limit_Vp_change(self, cell_type: int, Vp, stage, eta, dx: float, dep_ero: int):
        """Limit change in volume to a fraction of a cell volume.

        Function is used by multiple pathways in `mud_dep_ero` and
        `sand_dep_ero` but with different inputs for the sediment
        volume (:obj:`Vp`).

        For non-ocean cells the cap is 1/4 of the local depth (original
        behaviour).  For ocean cells the cap fraction is ``dx / (3 * hb)``
        where *hb* is the basin depth, allowing more deposition in
        shallower basins relative to the grid resolution.

        dep_ero indicates which pathway we are in, dep==0, ero==1
        """
        depth = stage - eta
        ocean = (cell_type == 0)
        # ocean fraction: dx / (3 * hb)
        ocean_frac = dx / (3.0 * self._hb)
        if depth < 0:
            if dep_ero == 0:
                return 0
            else:
                if ocean:
                    cap = np.abs(depth) * ocean_frac * (dx * dx)
                    return np.minimum(Vp, cap)
                else:
                    fourth = np.abs(depth) / 4 * (dx * dx)
                    return np.minimum(Vp, fourth)
        else:
            if ocean:
                cap = depth * ocean_frac * (dx * dx)
                return np.minimum(Vp, cap)
            else:
...
class SandRouter(BaseRouter):
    ...
    def _deposit_or_erode(self, px: int, py: int) -> None:
        ...
        if qs_loc > qs_cap:
            ...
            #     transport capacity of the cell (`qs_cap`), sediment needs to
            #     deposit on the bed.
            Vp_change = self._limit_Vp_change(self.cell_type[px, py],

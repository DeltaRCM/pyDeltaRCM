...
class Test__init__:
    ...
    def test_override_single_default(self, tmp_path: Path) -> None:
        ...
        utilities.write_parameter_to_file(f, "S0", 0.00018)
        f.close()
        delta = DeltaModel(input_file=p)
        assert delta.S0 == 0.00018
    ...
    def test_override_single_default_kwarg(self, tmp_path: Path) -> None:
        ...
        delta = DeltaModel(out_dir=_out_dir, S0=0.00018)
        assert delta.S0 == 0.00018

    def test_override_two_defaults_kwargs(self, tmp_path: Path) -> None:
        _out_dir = str(tmp_path / "out_dir")
        delta = DeltaModel(out_dir=_out_dir, S0=0.00018, Np_sed=2)
        assert delta.S0 == 0.00018
        assert delta.Np_sed == 2
    ...
    def test_override_yaml_with_kwarg(self, tmp_path: Path) -> None:
        ...
        utilities.write_parameter_to_file(f, "S0", 0.00018)
        f.close()
        with pytest.warns(UserWarning, match=r"A keyword specification"):
            delta = DeltaModel(input_file=p, S0=0.00012)
        assert delta.S0 == 0.00012
    ...
    def test_negative_active_layer_thickness(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"active_layer_thickness": -2.2}
        )
        with pytest.raises(ValueError):
            _ = DeltaModel(input_file=p)
    ...
    def test_clobber_netcdf(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(tmp_path, "input.yaml", {"clobber_netcdf": True})
    ...
    def test_clobbering(self, tmp_path: Path) -> None:
        p = utilities.yaml_from_dict(
            tmp_path, "input.yaml", {"clobber_netcdf": True, "save_eta_grids": True}
        )
...
class TestPublicSettersAndGetters:
    ...
    def test_getter_nosetter_bed_elevation(self, tmp_path: Path) -> None:
        ...
        # setter should return error, not allowed
        with pytest.raises(AttributeError):
            _delta.bed_elevation = np.random.uniform(0, 1, size=_delta.eta.shape)

...
def test_sed_route_padding(test_DeltaModel):
    ...
    """
    test_DeltaModel.pad_cell_type = np.pad(test_DeltaModel.cell_type, 1, 'edge')
    ...
    assert np.all(test_DeltaModel.pad_depth[1:-1, 1:-1] == test_DeltaModel.depth)
...
def test_sand_route_updates(test_DeltaModel):
    ...
    test_DeltaModel.sand_route()

    assert test_DeltaModel.Vp_res != test_DeltaModel.Vp_sed

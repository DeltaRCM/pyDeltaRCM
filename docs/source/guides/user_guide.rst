...
..
    ...
    Output files
    =======================
    ** Output files guide is imported from another file. **
...
Calculation of stratigraphy is not included in `pyDeltaRCM`. Computing
stratigraphy during model runtime is computationally expensive, and does not
affect the model runtime (:doc:`Sand and mud are not differentiated in eroded
sediment volume </info/morphodynamics>`.) 

If you would like to explore and understand stratigraphy generated by the
pyDeltaRCM model, please see the *sandpiper toolchain* project `sandplover
<https://sandpiper-toolchain.org/sandplover>`_, which seamlessly ingests
pyDeltaRCM output files and has easy-to-use tools for the stratigraphic
analyses.
